...
class HerokuCore(object):
    ...
    def _get_resource(self, resource, obj, params=None, **kwargs):
        ...
        return self._process_item(
            self._resource_deserialize(r.content.decode("utf-8")), obj, **kwargs
        )
...
class Heroku(HerokuCore):
    ...
    def oauthtoken_create(
        self, client_secret=None, grant_code=None, grant_type=None, refresh_token=None
    ):
        """
    ...
    def stream_app_log(
        self, app_id_or_name, dyno=None, lines=100, source=None, timeout=False
    ):
        logger = self._app_logger(
            app_id_or_name, dyno=dyno, lines=lines, source=source, tail=True
        )
    ...
    def get_app_log(
        ...
        logger = self._app_logger(
            app_id_or_name, dyno=dyno, lines=lines, source=source, tail=0
        )
